...
public class PacketSync extends PacketGenderInfo {
    ...
    public PacketSync(ByteBuf buffer) {
        super(buffer);
    }

    public static void handle(final PacketSync packet, Supplier<NetworkEvent.Context> context) {
        context.get().enqueueWork(() -> {
            if (Minecraft.getMinecraft().player == null || !packet.uuid.equals(Minecraft.getMinecraft().player.getUUID(Minecraft.getMinecraft().player.getGameProfile()))) {
